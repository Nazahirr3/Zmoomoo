...
var settingsButtonTitle = settingsButton ? settingsButton.getElementsByTagName("span")[0] : null;
...
ytLink.className = 'ytLink';
ytLink.href = tmpYoutuber.link;
var ytIcon = document.createElement('i');
ytIcon.className = 'material-icons';
ytIcon.style.verticalAlign = 'top';
ytIcon.textContent = '\uE064';
ytLink.appendChild(ytIcon);
ytLink.appendChild(document.createTextNode(' ' + tmpYoutuber.name));
featuredYoutuber.innerHTML = '';
featuredYoutuber.appendChild(ytLink);
...
function showLoadingText(text) {
    ...
    reloadLink.href = 'javascript:window.location.href=window.location.href';
...
function toggleSettings() {
    if (guideCard.classList.contains("showing")) {
        ...
        if (settingsButtonTitle) settingsButtonTitle.innerText = "Settings";
    } else {
        guideCard.classList.add("showing");
        if (settingsButtonTitle) settingsButtonTitle.innerText = "Close";
    }
}
...
function updateSkinColorPicker() {
    ...
    for (var i = 0; i < config.skinColors.length; ++i) {
        (function(index) {
            ...
            colorDiv.className = index == skinColor ? 'skinColorItem activeSkin' : 'skinColorItem';
            colorDiv.style.backgroundColor = config.skinColors[index];
            colorDiv.textContent = ' ';
            colorDiv.onclick = function() { selectSkinColor(index); };
...
function keyDown(event) {
    ...
    if (keyNum == 27) {
        ...
        if (!keys[keyNum]) {
            keys[keyNum] = 1;
            var customAction = getCustomKeyAction(keyNum);
            if (customAction) {
                if (customAction === 'moveUp') {
                    keys[87] = 1;
                    sendMoveDir();
                } else if (customAction === 'moveDown') {
                    keys[83] = 1;
                    sendMoveDir();
                } else if (customAction === 'moveLeft') {
                    keys[65] = 1;
                    sendMoveDir();
                } else if (customAction === 'moveRight') {
                    keys[68] = 1;
                    sendMoveDir();
                } else if (customAction === 'autoAttack') {
                    sendAutoGather();
                } else if (customAction === 'lockRotation') {
                    sendLockDir();
                } else if (customAction === 'gatherAttack') {
                    attackState = 1;
                    sendAtckState();
                    updateClientCps();
                } else if (customAction === 'quickFood') {
                    selectToBuild(player.items[0]);
                } else if (customAction === 'spike') {
                    var spikeId = player.items.find(function(id) { return id === 6 || id === 7 || id === 8 || id === 9; });
                    if (spikeId !== undefined) selectToBuild(spikeId);
                } else if (customAction === 'woodwall') {
                    var wallId = player.items.find(function(id) { return id === 3 || id === 4 || id === 5; });
                    if (wallId !== undefined) selectToBuild(wallId);
                } else if (customAction === 'trapBoostpad') {
                    var trapId = player.items.find(function(id) { return id === 15 || id === 16; });
                    if (trapId !== undefined) selectToBuild(trapId);
                } else if (customAction === 'turretTeleporter') {
                    var turretId = player.items.find(function(id) { return id === 17 || id === 22; });
                    if (turretId !== undefined) selectToBuild(turretId);
                } else if (customAction === 'windmill') {
                    var windmillId = player.items.find(function(id) { return id === 10 || id === 11 || id === 12; });
                    if (windmillId !== undefined) selectToBuild(windmillId);
                } else if (customAction === 'openShop') {
                    toggleStoreMenu();
...
var customKeybinds = {};
var activeKeybindInput = null;

var invalidKeybindActions = [];

function loadKeybinds() {
    try {
        var saved = localStorage.getItem('zmoomoo_keybinds');
        if (saved) {
            var parsed = JSON.parse(saved);
            for (var key in parsed) {
                if (invalidKeybindActions.indexOf(key) === -1) {
                    customKeybinds[key] = parsed[key];
                }
            }
            saveKeybinds();
        }
...
function saveKeybinds() {
    try {
        localStorage.setItem('zmoomoo_keybinds', JSON.stringify(customKeybinds));
    } catch (e) {}
}
...
function resetAllKeybinds() {
    customKeybinds = {};
    localStorage.removeItem('zmoomoo_keybinds');
    var inputs = document.querySelectorAll('.keybindInput');
...
function initKeybindInputs() {
    var inputs = document.querySelectorAll('.keybindInput');
    var resetBtn = document.getElementById('resetKeybinds');
    if (resetBtn) {
        resetBtn.addEventListener('click', resetAllKeybinds);
    }
    inputs.forEach(function(input) {
        ...
        if (customKeybinds[action]) {
            input.value = getKeyName(customKeybinds[action]);
        }
        input.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            if (activeKeybindInput) {
                activeKeybindInput.style.borderColor = '#e0e0e0';
            }
            activeKeybindInput = input;
            input.value = '...';
            input.style.borderColor = '#7ee559';
        });
...
function getCustomKeyAction(keyCode) {
    for (var action in customKeybinds) {
        if (customKeybinds[action] === keyCode) {
            return action;
        }
    }
    return null;
...
loadKeybinds();
document.addEventListener('DOMContentLoaded', initKeybindInputs);
if (document.readyState !== 'loading') {
    initKeybindInputs();
}
